...
#define PSTEPS 11 // Num streamline steps from start, for forward or back
#define HANNING 0 // Set to 1 to use hanning kernel, 0 for boxcar
#define L 20 // convolution kernel length
           ...
           double *segs, int m_idx, int back)
{
    ...
    // printf("line start = %f\n", line[m_idx - 1][0]);
    double Px = line[m_idx - 1][0];
    double Py = line[m_idx - 1][1];
    ...
    if ((s_top <= 0) && (s_bot <= 0) && (s_right <= 0) && (s_left <= 0)) {
            s = fabs(s_top);
        if ((fabs(s_bot) < s) && (s_bot != 0)) {
            s = fabs(s_bot);
        } else if ((fabs(s_right) < s) && (s_right != 0)) {
            s = fabs(s_right);
        } else if ((fabs(s_left) < s) && (s_left != 0)) {
            s = fabs(s_left);
        }
    // if any are positive, use positive min
    } else {
        s = 100;
        if ((s_top > 0) && (fabs(s_top) <= s)) { s = fabs(s_top); }
        if ((s_bot > 0) && (fabs(s_bot) <= s)) { s = fabs(s_bot); }
        if ((s_right > 0) && ( fabs(s_right) <= s)) { s = fabs(s_right); }
        if ((s_left > 0) && ( fabs(s_left) <= s)) { s = fabs(s_left); }
    }
    // s += 0.08;
    double new_Px = line[m_idx - 1][0] + ((dx/mag)*s);
    double new_Py = line[m_idx - 1][1] + ((dy/mag)*s);
    if ((fabs(new_Px - line[m_idx - 1][0]) > 1.5) ||
        (new_Px > m_field->xsize) || (new_Px < 0)) {
        return -1;
    } else if ((fabs(new_Py - line[m_idx - 1][1]) > 1.5) ||
        (new_Py > m_field->ysize) || (new_Py < 0)) {
        return -1;
    } else {
        line[m_idx][0] = new_Px;
        line[m_idx][1] = new_Py;
