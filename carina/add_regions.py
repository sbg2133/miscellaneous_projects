...
def car_regions(ax, f, wcs, c = 'white', ls = '-', add_text = True, add_dots = False):
    regs = []
    ...
    #       angle= 25 * u.deg).to_pixel(wcs)
    regs.append(sb.as_artist(facecolor='none', edgecolor=c, linestyle = ls, lw=2))
    ...
    regs.append(tr16.as_artist(facecolor='none', linestyle = ls, edgecolor=c, lw=2))
    ...
    regs.append(tr14.as_artist(facecolor='none', edgecolor=c, linestyle = ls, lw=2))
    ...
    regs.append(nb.as_artist(facecolor='none', linestyle = ls, edgecolor=c, lw=2))
    ...
    regs.append(sp.as_artist(facecolor='none', linestyle = ls, edgecolor=c, lw=2))
    ...
    if add_text:
        ...
        f.add_label(ra_14 - 0.01, dec_14, text = '2', size = 20, color = 'white')
        f.add_label(ra_16 + 0.08, dec_16, text = '3', size = 20, color = 'white')

    if add_dots:
        sightlines = np.load('tab_coords.npy')
        for sl in sightlines:
            coord = SkyCoord(sl[0], sl[1], unit='deg', frame='fk5')
            dot = reg.CircleSkyRegion(center=coord, radius=1 * u.arcmin).to_pixel(wcs)
            put = dot.as_artist(facecolor='red', edgecolor='white', fill = True)
            ax.add_patch(put)
    return
