...
def plot_vectors(ax, vectors, ys, xs, nskip = 1, alph = 1, col = 'white', pot = False):
    ...
    q = ax.quiver(xs[skip], ys[skip], xcomp, ycomp, units = 'x', color = col)#angles = 'xy', units = 'xy', scale = 1,\
...
def new_P(idx, vectors, start, temp_pos, ys, xs, back = False):
    ...
    if (new_Px < np.min(xs)) or (new_Px > np.max(xs)) or\
          (np.abs(new_Px - temp_pos[idx - 1][0]) > 1.1):
        return temp_pos, None
    if (new_Py < np.min(ys)) or (new_Py > np.max(ys)) or\
          (np.abs(new_Py - temp_pos[idx - 1][1]) > 1.1):
        return temp_pos, None
