...
pol_disp = np.load('large_files/pol_disp_CO13_5.npy')
#pol_disp = np.load('pol_disp_5arcmin.npy')
#pol_disp = np.rad2deg(pol_disp)
#v_array = np.load('large_files/v_array_CO13_5.npy')
v_fwhm = np.load('large_files/fwhmCO13_5.npy')
...
I, Q, U, wcs, pvals, psi = getPsi(blast_file, 0)
...
for i in range(len(pol_coords)):
    ...
    # nH2 = g/cm^3, V = km/s
    B[y,x] = 9.3*np.sqrt(2.0*nH2[y,x]) *\
           (vdisp_map[vy,vx] / (np.sqrt(2.0*pol_disp[y,x]**2)))
...
pretty()
plt.savefig(os.path.join(save_files_here, 'CO13_vselect_5arcmin.png'),\
...
f.show_colorscale(cmap = 'inferno', vmin = 100, smooth = 3,\
...
plt.savefig(os.path.join(save_files_here, 'cftCO13_5arcmin.png'), format='png', bbox_inches = 'tight')
